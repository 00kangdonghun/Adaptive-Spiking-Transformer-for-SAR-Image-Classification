...
# import additional modules for SAR data augmentation
from model import SpeckleNoise, RandomRotationWithPadding
...
parser.add_argument('-c', '--config', default='MSTAR.yml', type=str, metavar='FILE',
...
parser.add_argument('-T', '--time-step', type=int, default=4, metavar='time',           # => T=4 / 3 / 2 / 1
...
parser.add_argument('-data-dir', metavar='DIR',default="/home1/danny472/Adaptive-Spiking-Transformer-for-SAR-Image-Classification/dataset/MSTAR-10classes", #/dataset/ImageNet2012/
                    help='path to dataset')
parser.add_argument('--dataset', '-d', metavar='NAME', default='MSTAR-10classes',
...
def main():
    ...
    # ✨ Define SAR-specific data augmentation pipeline
    train_transforms = transforms.Compose([
        transforms.RandomApply([SpeckleNoise(variance=0.05)], p=0.5),
        transforms.RandomApply([RandomRotationWithPadding(degrees=15)], p=0.5),
    ])
    ...
    dataset_train = create_dataset(
        ...
        transform=train_transforms  # ✨ Add transform argument
    )
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            # ✨ Modify loss calculation method
            # Sum the loss for the final output + loss for all auxiliary outputs.
            total_loss = 0
            for i, output in enumerate(outputs):
                # Apply a larger weight to the final output (e.g., 1.0)
                # and apply smaller weights to intermediate auxiliary outputs (e.g., 0.5).
                loss_weight = 1.0 if i == len(outputs) - 1 else 0.5
                total_loss += loss_weight * loss_fn(output, target)
        
        # ✨ Replace the loss variable with total_loss.
        loss = total_loss
