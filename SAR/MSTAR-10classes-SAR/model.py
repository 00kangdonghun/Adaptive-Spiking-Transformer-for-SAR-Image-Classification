...
class vit_snn(nn.Module):
    ...
    def forward_features(self, x):  # x: [T,B,C,H,W]
        ...
        for i, blk in enumerate(self.block):
            ...
            # Pass through the auxiliary classifier only if it's not the last block
            if i < self.depths - 1:
                # [T,B,C,H,W] -> [T,B,C] (GAP over spatial) -> [B,C] (Time average)
                aux_x = x.flatten(3).mean(3)
                ...
                if not self.training:
                    ...
                    if (confidence > self.halting_th).all():
                        return [aux_output]

        # Case where it passed through all final blocks
        final_x = x.flatten(3).mean(3)
        intermediate_outputs.append(self.head(final_x.mean(0)))
    ...
    def forward(self, x):  # x: [B,C,H,W]
        ...
        # Receives all outputs during training, and the final (or early-exited) output during inference
        all_outputs = self.forward_features(x)
        
        # During training, return all outputs as a list to calculate each loss
        if self.training:
            return all_outputs
        # During inference, return only the last (or only) output
        else:
...
if __name__ == '__main__':
    ...
    model.train()
    y_train = model(x)
    print(f"Train mode output counts: {len(y_train)}") 
    print(f"Last output shape: {y_train[-1].shape}")
