...
class SpikingTokenizer(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=8,
...
class vit_snn(nn.Module):
    def __init__(self,
                 img_size_h=128, img_size_w=128, patch_size=8, in_channels=3, num_classes=10,
                 embed_dims=384, num_heads=6, mlp_ratios=4, qkv_bias=False, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=8, sr_ratios=1, T=4, spike_scale=0.125, lif_tau=2.0, stages=None,       # => T=4 / 3 / 2 / 1
                 pretrained_cfg=None):
        super().__init__()
...
if __name__ == '__main__':
    ...
    model = create_model(
        ...
        T=4, spike_scale=0.125, lif_tau=2.0, stages=None                    # => T=4 / 3 / 2 / 1
    ).cuda()
    model.eval()
    y = model(x)
    print(y.shape)
