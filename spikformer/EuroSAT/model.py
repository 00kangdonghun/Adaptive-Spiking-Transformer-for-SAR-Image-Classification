...
class SPS(nn.Module):
    def __init__(self, img_size_h=64, img_size_w=64, patch_size=4, in_channels=2, embed_dims=256):
        super().__init__()
...
def spikformer(pretrained=False, **kwargs):
    model = Spikformer(
        ...
        patch_size=16, embed_dims=384, num_heads=6, mlp_ratios=2,               # => dim 384, num_heads 6  // dim 512, num_heads 8
        in_channels=3, num_classes=10, qkv_bias=False,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=4, sr_ratios=1,      # => depths 8, 4
        **kwargs
    )
